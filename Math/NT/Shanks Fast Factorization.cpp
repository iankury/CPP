...
// Disable SDL checks: Properties -> C/C++ -> General -> SDL Checks
...
namespace NT {
  ...
  const static vector<uint32_t> multipliers = { 1, 3, 5, 7, 11, 3 * 5, 3 * 7, 3 * 11, 5 * 7, 5 * 11, 7 * 11, 3 * 5 * 7, 3 * 5 * 11, 3 * 7 * 11, 5 * 7 * 11, 3 * 5 * 7 * 11 };

  template<typename int_t = unsigned long long>
  struct Mod_Int {
    static inline int_t add(int_t const& a, int_t const& b, int_t const& M) {
      int_t ret = a + b;
      if (ret >= M) ret -= M;
      return ret;
    }
    static inline int_t sub(int_t const& a, int_t const& b, int_t const& M) {
      return add(a, M - b);
    }
    static inline int_t mul(int_t const& a, int_t const& b, int_t const& M) {
      uint64_t ret = a * (uint64_t)b - (uint64_t)((long double)a * b / M - 1.1) * M;
      if (-ret < ret) {
        ret = M - 1 - (-ret - 1) % M;
      }
      else {
    ...
    static inline int_t pow(int_t const& a, int_t const& b, int_t const& M) {
      int_t ret = 1;
      int_t base = a;
      for (int i = 0; b >> i; ++i) {
        if ((b >> i) & 1) ret = mul(ret, base, M);
  ...
  uint64_t GCD(uint64_t _a, uint64_t _b) { return _b ? GCD(_b, _a % _b) : _a; }
  ...
  uint64_t squfof_iter_better(uint64_t const& x, uint64_t const& k, uint64_t const& it_max, uint32_t cutoff_div) {
    if (GCD((uint64_t)k, x) != 1) return GCD((uint64_t)k, x);
    saved.clear();
    ...
    if (q1 == 0) {
      uint64_t factor = GCD(x, (uint64_t)p1);
    ...
    if (j == it_max) return 1;

    uint64_t factor = GCD((uint64_t)q0, x);
  ...
  template<typename T>
  typename enable_if<is_integral<T>::value, vector<T>>::type factor_vec(T x) {
    vector<T> ret;
    ...
    if (x > 1) {
      ...
      while (!s.empty()) {
        ...
        if (!miller_rabin(x)) {
          T factor = squfof(x);
          if (factor == 1 || factor == x) return ret;
          s.push(factor);
          s.push(x / factor);
        }
        else {
  ...
  template<typename T>
  typename enable_if<is_integral<T>::value, unordered_map<T, int>>::type factor_map(T x) {
