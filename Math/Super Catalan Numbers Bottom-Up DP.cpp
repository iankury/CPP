...
const int DPMAX = 27;
string super_cat[DPMAX];
void Build_Super_Cat() {
  BigInt bi;
  string minuend, subtrahend;
  super_cat[1] = "1";
  super_cat[2] = "1";
  for (int i = 2; i < DPMAX - 1; i++) {
    minuend = bi.Multiply(super_cat[i], to_string(6 * i - 3));
    subtrahend = bi.Multiply(super_cat[i - 1], to_string(i - 2));
    super_cat[i + 1] = bi.Divide(bi.Subtract(minuend, subtrahend), to_string(i + 1)).Quotient;
  }
}
